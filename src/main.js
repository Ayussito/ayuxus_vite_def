...
const showSongs = (artist) => {
  ...
  //para cada cancion un boton que manda cosas al web socket
  artist.canciones.forEach((cancion) => {
    htmlC +=
      "<ion-item><p>" +
      cancion.nombreC +
      "</p> <ion-button onclick='sendMessage(\""+cancion.nombreC+"\")' style='margin-left:auto'>Enviar</ion-button></ion-item>";
  });
  htmlC += "</ion-list></div>";
